...
class pair_Dataset(Dataset):
    def __init__(self, path, transforms_sar, transforms_opt):
        ...
        self.all_img_sar = [os.path.join(self.path_sar, self.sar_name_list[i]) for i in range(len(self.sar_name_list))] # sar文件夹下影像路径列表
        self.all_img_opt = [os.path.join(self.path_opt, self.opt_name_list[i]) for i in range(len(self.opt_name_list))] # opt文件夹下影像路径列表
...
def main():
    ...
    pair_datas = pair_Dataset('./datasets/lat_mid/test', transform_opt, transform_opt) # 对sar和opt实行同样的变换
    ...
    #         image_out.save(
    #             os.path.join(f'sample_results_12.06/{args.timestep_respacing}/gen_opt', f'{i + count * args.batch_size}.png'))
    #         image_sar = Image.fromarray(arr_sar[i])
    #         image_sar.save(os.path.join(f'sample_results_12.06/{args.timestep_respacing}/cond_sar',
    #                                     f'{i + count * args.batch_size}.png'))
    #         image_opt = Image.fromarray(arr_opt[i])
    #         image_opt.save(
    #             os.path.join(f'sample_results_12.06/{args.timestep_respacing}/gt_opt', f'{i + count * args.batch_size}.png'))
    #     count += 1
    #     if args.class_cond:
    #         gathered_labels = [
...
def create_argparser():
    ...
    defaults = dict(
        ...
        batch_size=2,
        use_ddim=False,
        model_path="./log/model1220000.pt",
    )
